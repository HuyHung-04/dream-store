...
public class KhachHangController {
    ...
    @GetMapping("/search")
    public ResponseEntity<Page<KhachHang>> searchKhachHang(
            @RequestParam int trangThai,
            @RequestParam("ten") String ten,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "8") int size) {
        return ResponseEntity.ok(khachHangService.getAllKhachHangByTenAndTrangThai(trangThai,ten, page, size));
    }
